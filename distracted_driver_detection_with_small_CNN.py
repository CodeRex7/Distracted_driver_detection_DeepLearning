...
model=Sequential()
model.add(Convolution2D(nb_filter=32, nb_row=3, nb_col=3, input_shape=(150,150,3),dim_ordering="tf"))
...
model.add(Convolution2D(nb_filter=64, nb_row=3, nb_col=3,dim_ordering="tf"))
model.add(Activation('relu'))
model.add(MaxPooling2D(pool_size=(2,2),dim_ordering="tf"))
